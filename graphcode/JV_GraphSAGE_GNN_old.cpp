...
template <typename T> //works only for numeric datatypes
void Add(vector<T> &A,const vector<T> &B,double multiplier = 1)
/*
Description:
...
// Subtraction if multiplier is -1
template <typename T> //works only for numeric datatypes
void Add(vector<vector<T>> &A,const vector<vector<T>> &B,int multiplier = 1)
/*
Description:
    performs A + multiplier*B;
*/
{
    cout<<"Add2 called : \n\t";
    cout<<"A.size() = "<<A.size()<<" B.size() = "<<B.size()<<endl;
    cout<<"\tA[0].size() = "<<A[0].size()<<" B[0].size() = "<<B[0].size()<<endl;
...
template <typename T> //works only for numeric datatypes
void Add(vector<vector<vector<T>>> &A,const vector<vector<vector<T>>> &B,int multiplier = 1)
/*
Description:
    performs A + multiplier*B;
*/
...
class GNN
{
        ...
        public:
            ...
            Activation acti; //creating an instance of the activation class
            ...
            GNN(vector<int> &hidden_sizes,vector<int> &samp_sizes,string aggr_type,int input_size,int output_size,string h_acti,string op_acti, Dataset &ds) : data(ds)
            {
                ...
                training_vertices = data.train_indices;
            ...
            vector<double> forward(int current_vertex, int batch_size = 1, int test_mode = 0)
            {
                ...
                for(int i =0; i<layers.size(); i++)
                {
                    ...
                    Scale(agg_vector,factor); //scale the aggregation vector by the factor.

                    for(auto &neigh : sampled_neighbours)
                        Add(agg_vector,NH_data[neigh],factor); //add the feature of the sampled neighbour to the aggregation vector.

                    NH_data[cur_vertex_index] = agg_vector; //replace the feature of the current vertex with the aggregated feature.
                    
                    NH_data = Mul(NH_data,layers[i].weights,batch_start_index,NH_batch_size); //NH_size because the last row is the feature of the current vertex of interest in the neighbourhood.
                    
                    Add(NH_data,layers[i].biases); // xTw + b transformation
            ...
            void backprop(int current_vertex,vector<double> &y_pred)
            {
                ...
                for(int i=layers.size()-1;i>=0;i--) //iterate over each hidden layer backwards : backprop
                {
                    ...
                    if(i>0) // if a previous layer exists, back propagate.
                    {
                        ...
                        // dw_list[i] = std::move(Outer(all_His[i],grad_ai));
                        // db_list[i] = std::move(grad_ai); //gradient of biases = grad_ai

                        auto dw_cur = std::move(Outer(all_His[i],grad_ai));

                        Add(dw[i],dw_cur); //accumulate the gradients of weights
                        Add(db[i],grad_ai); //accumulate the gradients of biases
...
class SGD : public Optimiser
{
    public:
        ...
        void step(bool done = false)
        {

            for(int l=0;l<model.layers.size();l++)
            {
                Scale(model.dw[l],lr*(1+l2_param));
                Add(model.layers[l].weights,model.dw[l],-1);

                Scale(model.db[l],lr*(1+l2_param));
                Add(model.layers[l].biases,model.db[l],-1);
            }

            fill_with_zeros(model.dw);
            fill_with_zeros(model.db);
...
class MomentumGD : public Optimiser
{
    public:
        ...
        {
            fill_with_zeros(prev_uw);
            fill_with_zeros(prev_ub);
        ...
        void step()
        {
            auto uw = LinearCombination(prev_uw,momentum,model.dw,lr);
            auto ub = LinearCombination(prev_ub,momentum,model.db,lr);

            auto temp_uw = LinearCombination(uw,1,model.dw,lr*l2_param);
            auto temp_ub = LinearCombination(ub,1,model.db,lr*l2_param);
...
class RMSprop : public Optimiser
{
    public:
        ...
        void step()
        {
            ...
            cout<<"Vb Dims : \n";
            model.printDims(vb_denominator);
            ...
            dw_tmp = LinearCombination(dw_tmp,lr,model.dw,lr*l2_param);
            db_tmp = LinearCombination(db_tmp,lr,model.db,lr*l2_param);
       ...
       void step()
        {
            ...
            m_w = LinearCombination(m_w,beta1,model.dw,1-beta1);
            m_b = LinearCombination(m_b,beta1,model.db,1-beta1);
